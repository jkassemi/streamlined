class Streamlined::Context::RequestContext
  ...
  def active_record_order_option(model_name="")
    #model_for_order = ((sclass =  Class.const_get(model_name.to_sym).superclass.to_s) == "ActiveRecord::Base") ? model_name : sclass
    if sort_column && sort_order
      {:order => ["#{Class.const_get(model_name.to_sym).table_name}.#{sort_column}", sort_order].map{|x| x.tr(" ", "_")}.join(" ")}
    else
      {}
    end
  end
