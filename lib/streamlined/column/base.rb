...
class Streamlined::Column::Base
  ...
  attr_with_default :hide_in_list, 'false' 
  attr_with_default :hide_in_show, 'false' 
  ...
  def render_th(view, context)
    x = Builder::XmlMarkup.new
  # x.th(:class => "sortSelector", :scope => "col", :col => "#{ActiveSupport::Inflector.pluralize(model_underscore)}.#{sort_column}" ) do
  ...
  def is_displayable_in_context?(view, item)
    ...
    when :show
      !(hide_if_unassigned && (!item.nil? and item.send(self.name).blank?)) and !hide_in_show
    when :list
      !(hide_if_unassigned && (!item.nil? and item.send(self.name).blank?)) and !hide_in_list
    when :edit
    ...
    if not item.nil?
      ...
      return  column_answer && instance_answer
    else
