...
class Streamlined::Column::Association < Streamlined::Column::Base
  ...
  attr_with_default :association_order, '"id"'
  ...
  def items_for_select
    ...
    if associables.size == 1
      klass.find(:all,:order=>"#{klass.table_name}.#{association_order}")
    else
      items = {}
  ...
  def render_td_show(view, item)
    view.render_streamlined_file(show_view.partial_with_underscore, 
