...
namespace :streamlined do
  ...
  task :install_files => :environment do  
...
Streamlined.ui_for(<%= model %>) do
    ...
    unless FileTest.exist? File.join(Rails.root.to_s, 'app', 'streamlined')
      FileUtils.mkdir(File.join(Rails.root.to_s, 'app', 'streamlined'))
    end

    ENV['MODEL'].split(',').each do |model|
      file_name = "#{model.underscore}_ui.rb"

      unless FileTest.exist? File.join(Rails.root.to_s, 'app', 'streamlined', file_name)
          File.open(File.join(Rails.root.to_s, 'app', 'streamlined', file_name), "a") { |f|
