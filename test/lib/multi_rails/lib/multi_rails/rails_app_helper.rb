...
module MultiRails
  module RailsAppHelper
    ...
    class << self
      ...
      # Make sure we have STREAMLINED_RAILS_ROOT set - will try to find it dynamically if its not set.
      def set_STREAMLINED_RAILS_ROOT
        ...
        raise("Must have a valid STREAMLINED_RAILS_ROOT.") unless Object.const_defined?("STREAMLINED_RAILS_ROOT") && STREAMLINED_RAILS_ROOT
        STREAMLINED_RAILS_ROOT
      end
      ...
      def find_STREAMLINED_RAILS_ROOT_dir
      ...
      def rails_gem_version_file
        File.expand_path("#{STREAMLINED_RAILS_ROOT}/config/rails_version.rb")
      end
      ...
      def environment_file
        File.expand_path(File.join(STREAMLINED_RAILS_ROOT, "config/environment.rb"))
      end
      
      def vendor_rails
        "#{STREAMLINED_RAILS_ROOT}/vendor/rails"
      end
      
      def vendor_rails_off
        "#{STREAMLINED_RAILS_ROOT}/vendor/rails.off"
      end
    end
