...
describe "install hook" do
  ...
  it "should copy the multi_rails_runner to the script directory" do
    ...
    FileUtils.expects(:symlink).with("#{STREAMLINED_RAILS_ROOT}/vendor/plugins/multi_rails/bin/multi_rails_runner.rb", 
  ...
  it 'should make the file executable' do
    Object.any_instance.stubs(:puts).returns(nil)
    Object.any_instance.expects(:`).with("chmod +x #{STREAMLINED_RAILS_ROOT}/script/multi_rails_runner")
    File.stubs(:symlink)
    load_install
  end
